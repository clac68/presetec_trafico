(function(window, document){
  ...
  var inicio=function(){
    ...
    var DataTrafico = {
      ...
      getDataDiaria: function(){
        return dataDiaria;
      },
      procesaHtml:function(nombreArchivo, htmlData){
      ...
      AgregaDataXHora:function(DataxHora, nombreArchivo, i, central, ruta, ano, mes, dia, hora, dataTrafico){
        hora=parseInt(hora);
      ...
      CalculaTraficoDiario: function(DataxHora){
        ...
        dataDiaria=DataxHora;
      },
      getObjDataDiaria:function(){
      ...
      ValidaData: function(nombreArchivo, linea, central, ruta, ano, mes, dia, hora, dataTrafico){
      ...
      AgregaError: function(nombreArchivo, msgError){
        arrErrores.push(nombreArchivo + ":" + msgError);
      },
      AgregaWarning: function(nombreArchivo, msgWarning){
          arrWarnings.push(nombreArchivo + ":" + msgWarning);
      },
      init: function(){
        arrErrores=[];
        arrWarnings=[];
        ...
        dataDiaria = [];
        /*
        while(arrErrores.length>0){
          arrErrores.remove(0);
        }
        while(arrWarnings.length>0){
          arrWarnings.remove(0);
        }
        */
      }
    }
  ...
  if (typeof window.DataTrafico === 'undefined'){
    window.DataTrafico = inicio();
  }else {
    console.log("La librería DataTrafico está siendo llamada nuevamente");
  }
})(window, document);

function getObjHora(hora, data){
  var objHora = {hora: hora, data: data};
  return objHora;
}
function getObjRuta(ruta, objHora){
  var objRuta = {ruta: ruta, horas:[], stat:getObjEstadistica()};
  objRuta.horas.push(objHora);
  return objRuta;
}
function getObjCentral(central, objRuta){
  var objCentral = {central:central, rutas:[]};
  objCentral.rutas.push(objRuta);
  return objCentral;
}

function getObjFecha(fecha, objCentral){
  var objFecha={fecha:fecha, centrales:[] }
  objFecha.centrales.push(objCentral);
  return objFecha;
}

function getObjEstadistica(){
